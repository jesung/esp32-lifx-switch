...
static int createHeader(bulb_service_t *p_bulb, const packet_config_t *p_config, lx_protocol_header_t * p_header) {
    ...
    p_header->size = sizeof(lx_protocol_header_t) + p_config->payload_size;
    p_header->protocol = PROTOCOL_NUMBER;
    p_header->addressable = ADDRESSABLE;
    p_header->tagged = p_config->tagged;
    p_header->origin = ORIGIN;
    p_header->source = source_id;
    // frame address
    p_header->target[0] = (uint8_t)((p_bulb->target >> 0) & 0xFF);
    p_header->target[1] = (uint8_t)((p_bulb->target >> 8) & 0xFF);
    p_header->target[2] = (uint8_t)((p_bulb->target >> 16) & 0xFF);
    p_header->target[3] = (uint8_t)((p_bulb->target >> 24) & 0xFF);
    p_header->target[4] = (uint8_t)((p_bulb->target >> 32) & 0xFF);
    p_header->target[5] = (uint8_t)((p_bulb->target >> 40) & 0xFF);
    p_header->target[6] = (uint8_t)((p_bulb->target >> 48) & 0xFF);
    p_header->target[7] = (uint8_t)((p_bulb->target >> 56) & 0xFF);
    p_header->ack_required = p_config->ack_required;
    p_header->res_required = p_config->res_required;
    // all set to 0
    // protocol header
    p_header->type = p_config->type;
...
*/
    // copy payload
    if (*p_payload_size > 0) {
        *pp_payload = (uint8_t*)malloc(*p_payload_size);
      memcpy(*pp_payload, p_buffer + sizeof(*p_header), *p_payload_size);
    }
...
int discoverBulbs(bulb_service_t ***ppp_bulbs) {
    ...
    int counter = 0;
    int bulb_counter = 0;
    bulb_service_t **pp_bulbs = (bulb_service_t**)malloc(sizeof(bulb_service_t *) * (BULB_LIMIT + 1));
    *ppp_bulbs = pp_bulbs;
    while (true) {
    	...
    	if (res == 1) {
            ...
            bulb_service_t *p_bulbs = (bulb_service_t*)malloc(sizeof(bulb_service_t));
